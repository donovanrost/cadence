...
> **Current implementation:** The commands and `/v1` API below document the
> working Stage 1 boundary. The accepted
...
> will separate `/admin/v1` from `/provider/v1` and replace per-reservation TCP
> fields with Service and Delivery Profile references. Do not treat the current
> `/v1` shape as a compatibility promise.
