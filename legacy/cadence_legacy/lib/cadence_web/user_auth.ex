defmodule CadenceWeb.UserAuth do
  ...
  @remember_me_cookie "_cadence_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CadenceWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  @doc """
  Plug for routes that require the user to be a system admin.
  """
  def require_system_admin(conn, _opts) do
    if conn.assigns.current_scope && Scope.system_admin?(conn.assigns.current_scope) do
      conn
    else
      conn
  ...
  @doc """
  Plug for routes that require the user to have a current organization.
  """
  def require_organization(conn, _opts) do
    if conn.assigns.current_scope && conn.assigns.current_scope.current_organization do
      conn
    else
      conn
  ...
  @doc """
  Switches the current organization for the authenticated user.
  Returns {:ok, conn} or {:error, reason}.
  """
  def switch_organization(conn, organization_id) do
    scope = conn.assigns.current_scope

    case Scope.switch_organization(scope, organization_id) do
      {:ok, new_scope} ->
        {:ok,
         conn
