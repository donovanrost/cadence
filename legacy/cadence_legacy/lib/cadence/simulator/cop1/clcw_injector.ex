defmodule Cadence.Simulator.COP1.CLCWInjector do
  @moduledoc """
  ...
  def new(opts \\ []) do
    ...
    %__MODULE__{overrides: overrides, schedule: schedule}
  end

  @spec apply(t(), CLCW.t(), non_neg_integer() | nil) :: CLCW.t()
  def apply(%__MODULE__{} = injector, %CLCW{} = clcw, step \\ 0) do
    overrides = resolve_overrides(injector, step)
    struct(clcw, overrides)
  end
  ...
  defp schedule_overrides(schedule, step) do
    ...
    |> Enum.reduce(%{}, fn %{overrides: overrides}, acc ->
      Map.merge(acc, overrides)
    end)
  end
  ...
  defp normalize_value(key, value) when key in @bit_fields do
    case value do
      true -> 1
      false -> 0
      value -> parse_integer(value) || value
    end
  end
