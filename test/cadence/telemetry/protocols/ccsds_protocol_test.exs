defmodule Cadence.Protocols.CCSDS.SpacePacketProtocolTest do
  ...
  defp build_packet_with_crc(apid, seq_count, payload, algorithm, opts \\ []) do
    ...
    # This matches how SpacePacketProtocol writes packets
  ...
  describe "new/1" do
    ...
    test "accepts custom sync pattern" do
      ...
      state = SpacePacketProtocol.new(sync_pattern: custom_sync)
      assert state.sync_pattern == custom_sync
    end

    test "accepts CRC options" do
      state =
        SpacePacketProtocol.new(
          crc_enabled: true,
          crc_algorithm: :crc32,
          crc_endian: :little,
    ...
    test "normalizes string algorithm to atom" do
      state = SpacePacketProtocol.new(crc_enabled: true, crc_algorithm: "crc32")
      assert state.crc_algorithm == :crc32
    end

    test "normalizes string endian to atom" do
      state = SpacePacketProtocol.new(crc_enabled: true, crc_endian: "little")
      assert state.crc_endian == :little
    end

    test "normalizes string on_failure to atom" do
      state = SpacePacketProtocol.new(crc_on_failure: "disconnect")
      assert state.crc_on_failure == :disconnect
    end

    test "accepts default_apid option" do
      state = SpacePacketProtocol.new(default_apid: 42)
      assert state.default_apid == 42
    end

    test "accepts include_sync and discard_sync options" do
      state = SpacePacketProtocol.new(include_sync: false, discard_sync: false)
  ...
  describe "read_data/2 - basic packet extraction" do
    test "extracts single packet without CRC" do
      ...
      assert {:ok, [extracted], new_state} = SpacePacketProtocol.read_data(packet, state)
    ...
    test "extracts packet keeping sync when discard_sync is false" do
      state = SpacePacketProtocol.new(discard_sync: false)
      payload = "Hello, CCSDS!"
      packet = build_packet(100, 1, payload)

      assert {:ok, [extracted], _state} = SpacePacketProtocol.read_data(packet, state)
    ...
    test "extracts multiple packets from single buffer" do
      ...
      assert {:ok, packets, new_state} = SpacePacketProtocol.read_data(combined, state)
    ...
    test "buffers partial packet and extracts on next read" do
      ...
      assert {:ok, [], state} = SpacePacketProtocol.read_data(first_half, state)
      assert state.buffer == first_half

      # Second read - should extract
      assert {:ok, [_extracted], state} = SpacePacketProtocol.read_data(second_half, state)
    ...
    test "skips garbage data before sync pattern" do
      ...
      assert {:ok, [_extracted], state} = SpacePacketProtocol.read_data(data, state)
      assert state.packets_extracted == 1
    end

    test "handles empty buffer" do
      state = SpacePacketProtocol.new([])
      assert {:ok, [], _state} = SpacePacketProtocol.read_data(<<>>, state)
    end
  end

  describe "read_data/2 - CRC validation" do
    test "validates CRC-16 successfully" do
      state = SpacePacketProtocol.new(crc_enabled: true, crc_algorithm: :crc16_ccitt)
    ...
    test "validates CRC-32 successfully" do
      ...
      assert {:ok, [_extracted], new_state} = SpacePacketProtocol.read_data(packet, state)
      assert new_state.crc_failures == 0
    end
    ...
    test "skips packet on CRC failure with on_failure: :skip" do
      ...
      assert {:ok, [], new_state} = SpacePacketProtocol.read_data(packet, state)
    ...
    test "disconnects on CRC failure with on_failure: :disconnect" do
      ...
      assert {:disconnect, reason} = SpacePacketProtocol.read_data(packet, state)
    ...
    test "respects CRC endianness" do
      ...
      assert {:ok, [_extracted], _state} = SpacePacketProtocol.read_data(packet, state)
    end
  end

  describe "write_data/2 - building packets from raw payload" do
    test "builds complete packet from raw payload" do
      state = SpacePacketProtocol.new(default_apid: 50)
      payload = "Raw payload data"

      assert {:ok, [framed], new_state} = SpacePacketProtocol.write_data(payload, state)
    ...
    test "builds packet with CRC" do
      ...
      assert {:ok, [framed], _state} = SpacePacketProtocol.write_data(payload, state)
    ...
    test "increments sequence counter" do
      ...
      {:ok, _, state} = SpacePacketProtocol.write_data("packet1", state)
      assert state.sequence_count == 1

      {:ok, _, state} = SpacePacketProtocol.write_data("packet2", state)
      assert state.sequence_count == 2

      {:ok, _, state} = SpacePacketProtocol.write_data("packet3", state)
      assert state.sequence_count == 3
    end

    test "wraps sequence counter at 16384" do
      state = %{SpacePacketProtocol.new([]) | sequence_count: 16_383}

      {:ok, _, state} = SpacePacketProtocol.write_data("payload", state)
      assert state.sequence_count == 0
    end
  end
  ...
  describe "write_data/2 - framing pre-built packets" do
    test "detects pre-built packet and adds framing" do
      ...
      assert {:ok, [framed], _state} = SpacePacketProtocol.write_data(pre_built, state)
    ...
    test "pre-built packet is readable after framing" do
      write_state = SpacePacketProtocol.new(crc_enabled: true, crc_algorithm: :crc16_ccitt)
      read_state = SpacePacketProtocol.new(crc_enabled: true, crc_algorithm: :crc16_ccitt)

      payload = "Roundtrip test"
      pre_built = build_packet(100, 5, payload, include_sync: false)

      # Write: add sync + CRC
      {:ok, [framed], _} = SpacePacketProtocol.write_data(pre_built, write_state)

      # Read: should extract successfully
      {:ok, [extracted], _} = SpacePacketProtocol.read_data(framed, read_state)
  ...
  describe "roundtrip - write then read" do
    test "raw payload roundtrip without CRC" do
      ...
      {:ok, [framed], state} = SpacePacketProtocol.write_data(original_payload, state)
      {:ok, [extracted], _state} = SpacePacketProtocol.read_data(framed, state)
    ...
    test "roundtrip with all supported CRC algorithms" do
      for algorithm <- [:crc16_ccitt, :crc32, :crc16_xmodem] do
        state = SpacePacketProtocol.new(crc_enabled: true, crc_algorithm: algorithm)
        original = "Test for #{algorithm}"

        {:ok, [framed], state} = SpacePacketProtocol.write_data(original, state)
        {:ok, [extracted], _state} = SpacePacketProtocol.read_data(framed, state)
    ...
    test "pre-built packet roundtrip" do
      ...
      {:ok, [framed], _} = SpacePacketProtocol.write_data(pre_built, state)
      {:ok, [extracted], _} = SpacePacketProtocol.read_data(framed, state)
  ...
  describe "packet_format/0" do
    test "returns :ccsds" do
      assert SpacePacketProtocol.packet_format() == :ccsds
    end
  end

  describe "connect_reset/1" do
    test "clears buffer on reset" do
      state = %{SpacePacketProtocol.new([]) | buffer: "leftover data"}
      reset_state = SpacePacketProtocol.connect_reset(state)
  ...
  describe "state tracking" do
    test "tracks packets_extracted" do
      ...
      {:ok, _, state} = SpacePacketProtocol.read_data(packet1, state)
      assert state.packets_extracted == 1

      {:ok, _, state} = SpacePacketProtocol.read_data(packet2, state)
    ...
    test "tracks packets_written" do
      ...
      {:ok, _, state} = SpacePacketProtocol.write_data("First", state)
      assert state.packets_written == 1

      {:ok, _, state} = SpacePacketProtocol.write_data("Second", state)
    ...
    test "tracks crc_failures" do
      ...
      {:ok, [], state} = SpacePacketProtocol.read_data(bad_packet, state)
