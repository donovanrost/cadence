...
func verifyPacket(fields []string) {
	...
	requireEqual(id, libraryWire, external)

	decoded := ccsds.Packet{}
	reader := serial.NewReader(libraryWire)
...
func requireEqual(id string, libraryWire []byte, external []byte) {
	if !bytes.Equal(libraryWire, external) {
		fail("%s: bytes differ: ccsds=%s hermes=%s", id, upperHex(libraryWire), upperHex(external))
	}
}
