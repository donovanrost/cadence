...
Reproduction command, from `packages/ccsds`:

```sh
bash conformance/hermes/run.sh
```

For every case, this library first produces the wire bytes. The pinned Hermes codec
decodes those bytes, independently marshals the same semantic value, and
requires byte equality. This library then decodes the Hermes-produced bytes and
