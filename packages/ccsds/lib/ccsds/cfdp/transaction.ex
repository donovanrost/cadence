defmodule CCSDS.CFDP.Transaction do
  @moduledoc false

  alias CCSDS.CFDP.{PDU, TransactionID}

  @spec id(PDU.t()) :: TransactionID.t()
  def id(%PDU{} = pdu),
    do: TransactionID.new(pdu.source_entity_id, pdu.transaction_sequence_number)

  @spec pdu(map(), PDU.payload(), CCSDS.CFDP.direction()) :: PDU.t()
  ...
  @spec validate_incoming(PDU.t(), map(), CCSDS.CFDP.direction()) ::
          :ok | {:error, term()}
