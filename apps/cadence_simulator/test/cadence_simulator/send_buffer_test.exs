defmodule CadenceSimulator.SendBufferTest do
  ...
  test "refreshes runtime output before reconnecting to a stale TCP port" do
    ...
    {:ok, first_live_listener} = :gen_tcp.listen(0, [:binary, active: false, reuseaddr: true])
    {:ok, first_live_port} = :inet.port(first_live_listener)

    {:ok, second_live_listener} = :gen_tcp.listen(0, [:binary, active: false, reuseaddr: true])
    ...
    assert {:ok, "recovered"} == :gen_tcp.recv(second_socket, byte_size("recovered"), 1_000)
