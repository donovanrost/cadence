defmodule Cadence.Repo.Migrations.CreateDataSourceWatermarks do
  use Ecto.Migration

  def change do
    create table(:data_source_watermark_events, primary_key: false) do
    ...
    create(
      index(:data_source_watermark_events, [:source_watermark_key, :observed_at],
        name: :data_source_watermark_events_key_observed_idx
      )
    )

    create(
      index(:data_source_watermark_events, [:organization_id, :mission_id, :logical_source],
        name: :data_source_watermark_events_scope_idx
      )
    )

    create(
      index(:data_source_watermark_events, [:data_source_id],
        name: :data_source_watermark_events_data_source_idx
      )
    )

    create table(:data_source_watermark_statuses, primary_key: false) do
    ...
    create(
      index(
        :data_source_watermark_statuses,
        [:organization_id, :mission_id, :logical_source],
        name: :data_source_watermark_statuses_scope_idx
      )
    )

    create(
      index(:data_source_watermark_statuses, [:data_source_id],
        name: :data_source_watermark_statuses_data_source_idx
      )
    )
  end
end
