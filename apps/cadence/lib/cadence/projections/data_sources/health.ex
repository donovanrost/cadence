defmodule Cadence.Projections.DataSources.Health do
  @moduledoc """
  Durable source-health transition log and latest-status projection.

  It records the concrete source identity used by readers and publishes
  committed health facts without knowing which products consume them.
  """
  ...
  alias Cadence.DataSources.{Facts, SourceHealthEvent, SourceHealthStatus}
  ...
  def enabled?(opts \\ []) when is_list(opts) do
    case Keyword.fetch(opts, :source_health_events?) do
      {:ok, enabled?} -> enabled?
      :error -> data_source_health_events_enabled?()
    end
  end
  ...
  defp data_source_health_events_enabled? do
    :cadence
    |> Application.get_env(:data_source_health_events, [])
