...
defmodule Cadence.Accounts do
  ...
  @type user_session_context :: Authentication.user_session_context()
  ...
  defdelegate authenticate_user_session(session_token), to: Authentication
  ...
  def establish_organization_access(email, organization_id, opts \\ [])
    ...
    with {:ok, normalized_email} <- normalize_required_email(email),
      ...
      case {Authentication.durable_user_by_email(normalized_email), force_invitation} do
        {{:ok, %User{} = user}, false} ->
          result =
            grant_path(
              Repo,
  ...
  def fetch_organization_invitation(invitation_token) when is_binary(invitation_token) do
    token_digest = Token.digest(invitation_token)
    ...
    with {:ok, acceptance_attrs} <- validate_invitation_acceptance_attrs(attrs) do
      ...
      Repo.transaction(fn ->
        with {:ok, invitation_row, invitation} <- lock_usable_invitation(Repo, token_digest),
             ...
             :ok <-
               Authentication.upsert_password_credential(
                 Repo,
                 user.user_id,
             ...
             :ok <- Authentication.revoke_all_user_sessions(Repo, user.user_id),
             {:ok, issued_session} <-
               Authentication.issue_browser_session(user, membership.organization_id) do
          %{
            user: user,
  ...
  defp grant_platform_admin_if_requested(repo, %User{} = user, true) do
    ...
    Authentication.upsert_user(repo, updated_user)
  end
  ...
  defp insert_invitation(
    ...
    invitation_token = Token.generate()
    ...
    invitation =
      OrganizationInvitation.new(%{
        ...
        status: :pending,
        token_digest: Token.digest(invitation_token),
        token_hint: Token.hint(invitation_token),
  ...
  defp upsert_invited_user(repo, %OrganizationInvitation{} = invitation, acceptance_attrs)
    ...
    case repo.get_by(UserRow, email: email) do
      nil ->
        ...
        Authentication.upsert_user(repo, user)
