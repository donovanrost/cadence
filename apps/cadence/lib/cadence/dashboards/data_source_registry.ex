defmodule Cadence.Dashboards.DataSourceRegistry do
  ...
  def facts(%PlannedSourceRequest{} = request, opts \\ []) when is_list(opts) do
    with {:ok, %ResolvedSourceBinding{} = resolved_binding} <- resolve(request, opts),
         {:ok, adapter} <- adapter_for(request, resolved_binding) do
  ...
  defp do_resolve(%PlannedSourceRequest{} = request, opts) do
    case registry_data(request, opts) do
      {:historical, data_sources, intervals} ->
        resolve_historical(request, data_sources, intervals, opts)
  ...
  defp do_resolve_segments(%PlannedSourceRequest{} = request, opts) do
    case registry_data(request, opts) do
      {:historical, data_sources, intervals} ->
        resolve_historical_segments(request, data_sources, intervals, opts)
  ...
  defp resolve_historical_segments(
         %PlannedSourceRequest{} = request,
         data_sources,
         intervals,
         opts
    ...
    case source_binding_range(opts) do
      ...
      nil ->
        with {:ok, resolved_binding} <- resolve_historical(request, data_sources, intervals, opts) do
          {:ok, [resolved_binding]}
        end
    end
  end
  ...
  defp adapter_for(%PlannedSourceRequest{} = request, %ResolvedSourceBinding{
         data_source: %DataSource{adapter: adapter}
       })
  ...
  defp adapter_for(%PlannedSourceRequest{} = request, %ResolvedSourceBinding{}) do
    {:error,
     warning(
       request,
       :unsupported_source_adapter,
