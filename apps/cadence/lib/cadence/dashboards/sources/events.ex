defmodule Cadence.Dashboards.Sources.Events do
  ...
  * mission timeline annotations from the `mission_events` projection
  ...
  defp ensure_events_source(%PlannedSourceRequest{logical_source: :events}), do: :ok

  defp ensure_events_source(%PlannedSourceRequest{} = request) do
    {:warning,
     warning(
       request,
  ...
  defp ensure_supported_sampling(%PlannedSourceRequest{} = request) do
  ...
  defp requested_products(%PlannedSourceRequest{} = request) do
    requested =
      request.sampling
      |> get_attr(:products)
      |> case do
  ...
  defp time_axis_warnings(%PlannedSourceRequest{} = request) do
  ...
  defp common_meta(%PlannedSourceRequest{} = request, source_binding) do
    %{
  ...
  defp mission_timeline_projection(%PlannedSourceRequest{} = request) do
    if replay_run_id(request), do: :operational_events, else: :mission_events
  end

  defp contact_interval_projection(%PlannedSourceRequest{} = request) do
  ...
  defp time_window(%PlannedSourceRequest{time_context: time_context}) do
    from =
  ...
  defp request_limit(%PlannedSourceRequest{sampling: sampling}) do
  ...
  defp dataset(source_binding), do: get_attr(source_binding, :dataset)
  ...
  defp source_endpoint_scope_id(%PlannedSourceRequest{} = request) do
    if ScopeContext.primary_kind(request.scope_context) in [:source_endpoint, "source_endpoint"] do
