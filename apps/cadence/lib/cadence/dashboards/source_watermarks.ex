defmodule Cadence.Dashboards.SourceWatermarks do
  @moduledoc """
  ...
  def record_source_watermark(attrs, opts \\ []) when is_map(attrs) and is_list(opts) do
    ...
    |> case do
      {:ok, {:changed, event, status}} ->
        maybe_invalidate_source_watermark(event, opts)
        {:ok, event, status}
  ...
  def enabled?(opts \\ []) when is_list(opts) do
    case Keyword.fetch(opts, :source_watermark_events?) do
      {:ok, enabled?} -> enabled?
      :error -> dashboard_source_watermark_events_enabled?()
    end
  end
  ...
  defp maybe_invalidate_source_watermark(%SourceWatermarkEvent{} = event, opts) do
    if Keyword.get(opts, :invalidate_runtime_cache?, true) do
      RuntimeInvalidation.source_watermark_changed(
        %{
          ...
          evidence_ref: %{
            kind: "dashboard_source_watermark_event",
  ...
  defp dashboard_source_watermark_events_enabled? do
    :cadence
    |> Application.get_env(:dashboard_source_watermark_events, [])
