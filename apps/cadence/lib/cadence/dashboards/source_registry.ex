defmodule Cadence.Dashboards.SourceRegistry do
  ...
  defp capability_provenance(
    ...
    provenance =
      %{
        ...
        capability_posture: capability_posture(request, capabilities),
  ...
  defp capability_posture(%PlannedSourceRequest{} = request, %SourceCapabilities{} = capabilities) do
  ...
  defp segmented_facts(%PlannedSourceRequest{} = request, resolved_bindings, opts) do
    ...
    |> case do
      {:ok, segment_facts} -> {:ok, merge_segment_facts(request, segment_facts)}
  ...
  defp merge_segment_facts(%PlannedSourceRequest{} = request, segment_facts) do
    segments =
      Enum.map(segment_facts, fn {resolved_binding, _facts} ->
        source_binding_segment_metadata(resolved_binding)
      end)
    ...
    SourceFacts.new(%{
      ...
      meta: %{
        ...
        source_binding_segment_count: length(segments),
        source_binding_segments: segments,
        source_health_by_segment: source_health_by_segment(segment_facts),
        capability_posture: aggregate_capability_posture(facts),
        capability_posture_by_segment: capability_posture_by_segment(segment_facts),
  ...
  defp source_health_by_segment(segment_facts) do
    Enum.map(segment_facts, fn {resolved_binding, %SourceFacts{} = facts} ->
      resolved_binding
      |> source_binding_segment_metadata()
  ...
  defp capability_posture_by_segment(segment_facts) do
    segment_facts
  ...
  defp resolve_segmented_source(
    ...
    case merge_segment_results(request, segment_results) do
  ...
  defp merge_segment_results(%PlannedSourceRequest{} = request, segment_results) do
    ...
    segment_metadata =
      Enum.map(segment_results, fn {binding, _result} ->
        source_binding_segment_metadata(binding)
      end)
