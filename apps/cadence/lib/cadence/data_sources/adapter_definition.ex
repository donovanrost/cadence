defmodule Cadence.DataSources.AdapterDefinition do
  @moduledoc "Typed registry entry for one configured Data Source adapter."
  ...
  def validate(%__MODULE__{} = definition) do
    with true <- is_atom(definition.logical_source) and not is_nil(definition.logical_source),
         ...
         true <- valid_probe_module?(definition.probe_module),
  ...
  defp valid_probe_module?(nil), do: true

  defp valid_probe_module?(module) when is_atom(module),
    do: Code.ensure_loaded?(module) and function_exported?(module, :probe, 2)

  defp valid_probe_module?(_module), do: false
