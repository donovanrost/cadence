defmodule Cadence.Management.DataSources.Store.DataSourceRow do
  @moduledoc false
  ...
  schema "data_source_definitions" do
    field(:owner, :string)
    field(:kind, :string)
  ...
  def changeset(%__MODULE__{} = row, %DataSource{} = data_source) do
    ...
    |> foreign_key_constraint(:credentials_ref, name: :data_source_definitions_credentials_ref_fk)
    |> foreign_key_constraint(:organization_id, name: :data_source_definitions_org_fk)
    |> foreign_key_constraint(:mission_id, name: :data_source_definitions_org_mission_fk)
  end
