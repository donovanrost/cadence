defmodule Cadence.Management.DataSources.Store do
  @moduledoc """
  Persistence boundary for desired data-source definitions and bindings.
  """
  ...
  def ensure_default_managed_sources! do
    with {:ok, data_source} <- persist_data_source(default_managed_data_source()),
      ...
      {:error, reason} ->
        raise "failed to bootstrap data sources: #{inspect(reason)}"
    end
  end
  ...
  defp validate_data_source_credentials(%DataSource{} = data_source) do
    case Credentials.resolve(data_source.credentials_ref,
