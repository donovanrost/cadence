defmodule Cadence.Smoke.DataSources.BYOQuestDB do
  @moduledoc """
  End-to-end smoke check for a customer-owned QuestDB data source.

  This smoke is intentionally above the storage adapter smoke. It proves that a
  customer-owned data source can resolve runtime credential material,
  ...
  alias Cadence.Dashboards.{PlannedSourceRequest, SourceRegistry}
  alias Cadence.DataSources.{DataBinding, DataSource, SourceCredentialMaterial}
  alias Cadence.Management.DataSources
  alias Cadence.Management.DataSources.Credentials, as: SourceCredentials
  alias Cadence.Management.DataSources.Credentials.EnvMaterialResolver
  ...
  @default_credentials_ref "secret://questdb-byo-smoke/data-sources/customer-questdb"
  ...
  defp cleanup_queries(context) do
    [
      ...
      {"DELETE FROM data_source_health_statuses WHERE data_source_id = $1 OR source_health_event_id IN (SELECT source_health_event_id FROM data_source_health_events WHERE data_source_id = $1)",
       [context.data_source_id]},
      {"DELETE FROM data_source_health_events WHERE data_source_id = $1",
       [context.data_source_id]},
      {"DELETE FROM data_source_binding_events WHERE binding_id = $1", [context.binding_id]},
      {"DELETE FROM data_source_bindings WHERE binding_id = $1", [context.binding_id]},
      {"DELETE FROM data_source_definition_events WHERE data_source_id = $1",
       [context.data_source_id]},
      {"DELETE FROM data_source_definitions WHERE data_source_id = $1", [context.data_source_id]},
      {"DELETE FROM data_source_credential_events WHERE credentials_ref = $1",
       [context.credentials_ref]},
      {"DELETE FROM data_source_credential_references WHERE credentials_ref = $1",
       [context.credentials_ref]}
    ...
    {"""
     ...
     WHERE subject_id IN ($1, $2, $3)
        OR source_record_id IN (
          ...
          FROM data_source_definition_events
          WHERE data_source_id = $1
        )
        OR source_record_id IN (
          SELECT data_binding_event_id
          FROM data_source_binding_events
          WHERE binding_id = $2
        )
        OR source_record_id IN (
          SELECT source_health_event_id
          FROM data_source_health_events
          WHERE data_source_id = $1
        )
        OR source_record_id IN (
          SELECT source_credential_event_id
          FROM data_source_credential_events
