defmodule Cadence.Catalog.Command.ArrayShape do
  @moduledoc """
  Canonical array shape for command argument type definitions.
  """

  alias Cadence.Catalog.Command.Normalize

  @type t :: %__MODULE__{
          element_argument_type_id: binary(),
  ...
  def new(attrs) when is_map(attrs) do
    %__MODULE__{
      element_argument_type_id: Normalize.fetch!(attrs, :element_argument_type_id),
  ...
  defp map_list(values) when is_list(values), do: Enum.filter(values, &is_map/1)
