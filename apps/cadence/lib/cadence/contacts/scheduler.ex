defmodule Cadence.Contacts.Scheduler do
  @moduledoc """
  Polling reconciler for scheduled and realized contact lifecycle state.
  """
  ...
  def handle_call({:reconcile_now, %DateTime{} = reference_time}, _from, state) do
    {:reply, Contacts.reconcile(reference_time), state}
  end

  @impl true
  def handle_info(:reconcile, state) do
    _ = Contacts.reconcile(resolve_reference_time(state))
    schedule_next_reconcile(state)
    {:noreply, state}
  end
