defmodule Cadence.Dashboards.DataSourcesTest do
  ...
  test "persists and lists dashboard data sources" do
    ...
    assert [listed] = DataSources.list_data_sources("org-dash-source", "mission-dash-source")
  ...
  test "QuestDB telemetry probes mark connection failures unavailable with diagnostics" do
    ...
    assert unavailable_status.source_health == :unavailable
    assert unavailable_event.payload["connection_test_result"] == "failed"
    assert unavailable_event.payload["connection_test_kind"] == "adapter_io"
    assert unavailable_event.payload["connection_test_message"] =~ "failed"
  ...
  test "rejects unsafe BYO data source configurations before persistence" do
    ...
    assert "must be customer_owned for BYO TSDB data sources" in field_errors(
             changeset,
             :isolation_level
           )
