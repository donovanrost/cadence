defmodule Cadence.Transport.TCStreamId do
  ...
  Stream identity is keyed by mission + channel (scid/vcid/map), independent of interface.
  """
  ...
  def to_string(%__MODULE__{} = tc_stream_id) do
    "mission_id=#{tc_stream_id.mission_id} scid=#{tc_stream_id.scid} vcid=#{tc_stream_id.vcid} dir=#{tc_stream_id.dir} map_id=#{inspect(tc_stream_id.map_id)} interface_id=#{inspect(tc_stream_id.interface_id)}"
  end
