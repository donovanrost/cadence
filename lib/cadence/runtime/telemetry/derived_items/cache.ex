defmodule Cadence.Runtime.Telemetry.DerivedItems.Cache do
  @moduledoc """
  ...
  @doc """
  Pre-warms the cache for a specific mission.

  Call this at mission startup to ensure derived item definitions are cached
  before telemetry processing begins. This avoids a database query on the first
  telemetry packet that triggers derived item computation.

  ## Example

      Cache.warm(mission_id)

  """
  @spec warm(String.t()) :: :ok
  def warm(mission_id) do
    _ = get_definitions(mission_id)
    :ok
  end
