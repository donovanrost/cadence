defmodule Cadence.Missions.MissionSupervisor do
  @moduledoc """
  ...
  def start_mission(%Mission{} = mission) do
    Logger.info("Starting mission supervision tree for mission_id=#{mission.id}")

    child_spec = {
      Cadence.Missions.MissionInstance,
      mission: mission
    }
    ...
    case DynamicSupervisor.start_child(__MODULE__, child_spec) do
      {:ok, pid} = result ->
        Logger.info(
          "Mission supervision tree started successfully for mission_id=#{mission.id}, pid=#{inspect(pid)}"
        )
      ...
      {:error, {:already_started, pid}} ->
        Logger.warning(
          "Mission supervision tree already running for mission_id=#{mission.id}, pid=#{inspect(pid)}"
        )
      ...
      {:error, reason} = error ->
        Logger.error(
          "Failed to start mission supervision tree for mission_id=#{mission.id}, reason=#{inspect(reason)}"
        )
