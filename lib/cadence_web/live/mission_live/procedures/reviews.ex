defmodule CadenceWeb.MissionLive.Procedures.Reviews do
  @moduledoc """
  Mission-scoped procedure reviews inbox.

  Provides a view of all review-related activity for the current user
  within a specific mission. Similar to the global reviews inbox but
  filtered to procedures belonging to this mission.
  ...
  def handle_params(params, _uri, socket) do
    ...
    # Load all inbox data filtered to this mission
    opts = [mission_id: mission.id, limit: 20]

    requested = Reviews.list_requested_reviews(user_id, opts)
    awaiting = Reviews.list_awaiting_rereview(user_id, opts)
    submissions = Reviews.list_user_submissions(user_id, opts)
    activity = Reviews.list_recent_activity(user_id, opts)
    counts = Reviews.get_inbox_counts(user_id, opts)

    section = parse_section(params["section"])

    {:noreply,
     socket
     |> assign(:page_title, "Procedure Reviews")
    ...
    <div class="p-6">
      ...
      <div class="mt-6">
        ...
        <div class="tabs tabs-boxed bg-base-200 p-1 mb-6">
          <.link
            patch={~p"/missions/#{@mission}/procedures/reviews"}
          ...
          <.link
            patch={~p"/missions/#{@mission}/procedures/reviews?section=awaiting"}
          ...
          <.link
            patch={~p"/missions/#{@mission}/procedures/reviews?section=submissions"}
          ...
          <.link
            patch={~p"/missions/#{@mission}/procedures/reviews?section=activity"}
        ...
        <%= case @active_section do %>
          <% :requested -> %>
            <.requested_reviews_section items={@requested_reviews} mission={@mission} />
          <% :awaiting -> %>
            <.awaiting_rereview_section items={@awaiting_rereview} mission={@mission} />
          <% :submissions -> %>
            <.submissions_section items={@submissions} mission={@mission} />
          <% :activity -> %>
            <.activity_section items={@recent_activity} mission={@mission} />
        <% end %>
      </div>
    </div>
    """
    ...
    <div class="space-y-4">
      ...
      <%= if Enum.empty?(@items) do %>
        <.empty_state
          ...
          description="You're all caught up! No one has requested a review from you for this mission."
        />
      <% else %>
        <div class="space-y-3">
          <.review_request_card :for={item <- @items} item={item} mission={@mission} />
        </div>
      <% end %>
    </div>
    """
  ...
  defp awaiting_rereview_section(assigns) do
    ...
    <div class="space-y-4">
      ...
      <% else %>
        <div class="space-y-3">
          <.rereview_card :for={item <- @items} item={item} mission={@mission} />
        </div>
      <% end %>
    </div>
    """
    ...
    <div class="space-y-4">
      ...
      <%= if Enum.empty?(@items) do %>
        <.empty_state
          ...
          description="You haven't submitted any procedure versions for review in this mission."
        />
      <% else %>
        <div class="space-y-3">
          <.submission_card :for={item <- @items} item={item} mission={@mission} />
        </div>
      <% end %>
    </div>
    """
    ...
    <div class="space-y-4">
      ...
      <%= if Enum.empty?(@items) do %>
        <.empty_state
          ...
          description="Your review activity for this mission will appear here."
        />
      <% else %>
        <div class="space-y-3">
          <.activity_card :for={item <- @items} item={item} mission={@mission} />
        </div>
      <% end %>
    </div>
    """
    ...
    <div class="card bg-base-200 hover:bg-base-200/80 transition-colors">
      <div class="card-body p-4">
        <div class="flex items-start justify-between gap-4">
          ...
          <.link
            navigate={procedure_review_path(@mission, @item.version)}
  ...
  defp procedure_review_path(mission, version) do
    procedure = version.procedure
    ~p"/missions/#{mission.id}/procedures-v2/#{procedure.id}/versions/#{version.id}/review"
  end
